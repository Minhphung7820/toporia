    ...
    public function getResults(): ModelCollection
    {
        ...
        if ($hasWhereIn && !$this->parent->exists()) {
            ...
            if ($limit !== null && $limit > 0) {
                ...
                $windowQuery = "SELECT * FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY {$wrappedParentPivotKey} ORDER BY {$orderByClause}) AS toporia_row FROM ({$baseQuerySql}) AS toporia_base WHERE {$wrappedParentPivotKey} IN ({$placeholders})) AS toporia_table WHERE {$rowFilter}";
