...
if (!function_exists('tenant')) {
    /**
     * Get the current tenant or tenant manager.
     *
     * Usage:
     * - tenant() - Get current tenant (or null)
     * - tenant()->getTenantKey() - Get tenant ID
     * - tenant()->getConfig('feature') - Get tenant config
     *
     * @return \Toporia\Framework\MultiTenancy\Contracts\TenantInterface|null
     */
    function tenant(): ?\Toporia\Framework\MultiTenancy\Contracts\TenantInterface
    {
        return \Toporia\Framework\MultiTenancy\TenantManager::current();
    }
}
...
if (!function_exists('tenant_id')) {
    ...
    {
        return \Toporia\Framework\MultiTenancy\TenantManager::id();
    }
}
...
if (!function_exists('has_tenant')) {
    ...
    {
        return \Toporia\Framework\MultiTenancy\TenantManager::check();
    }
}

if (!function_exists('run_as_tenant')) {
    /**
     * Run callback in context of specific tenant.
     *
     * @template T
     * @param \Toporia\Framework\MultiTenancy\Contracts\TenantInterface $tenant
     * @param callable(): T $callback
     * @return T
     */
    function run_as_tenant(
        \Toporia\Framework\MultiTenancy\Contracts\TenantInterface $tenant,
        callable $callback
    ): mixed {
        return \Toporia\Framework\MultiTenancy\TenantManager::run($tenant, $callback);
    }
}

if (!function_exists('run_without_tenant')) {
    /**
     * Run callback without tenant context (bypass tenant scoping).
     *
     * @template T
     * @param callable(): T $callback
     * @return T
     */
    function run_without_tenant(callable $callback): mixed
    {
        return \Toporia\Framework\MultiTenancy\TenantManager::runWithoutTenant($callback);
    }
}
...
if (!function_exists('api_version')) {
     ...
     *
     * Usage:
     * - api_version() - Get current version (e.g., 'v1')
     * - api_version('v2') - Check if current version is 'v2'
     *
     * @param string|null $check Version to check against
     * @return string|bool Current version or comparison result
     */
    function api_version(?string $check = null): string|bool
    {
        if ($check !== null) {
            return \Toporia\Framework\ApiVersioning\ApiVersion::is($check);
        }

        return \Toporia\Framework\ApiVersioning\ApiVersion::current();
    }
}

if (!function_exists('api_version_at_least')) {
    /**
     * Check if current API version is at least the given version.
     *
     * @param string $version Minimum version
     * @return bool
     */
    function api_version_at_least(string $version): bool
    {
        return \Toporia\Framework\ApiVersioning\ApiVersion::isAtLeast($version);
    }
}
...
if (!function_exists('audit')) {
     ...
     *
     * Usage:
     * - audit() - Get AuditManager instance
     * - audit()->record($model, 'updated', $old, $new)
     * - audit()->getHistory(UserModel::class, $userId)
     * - audit()->batch(fn($audit) => ...)
     *
     * @return \Toporia\Framework\Audit\AuditManager
     */
    function audit(): \Toporia\Framework\Audit\AuditManager
    {
        return app(\Toporia\Framework\Audit\AuditManager::class);
    }
}
...
if (!function_exists('audit_record')) {
     ...
     * @param array<string, mixed> $oldValues Old values
     * @param array<string, mixed> $newValues New values
     * @param array<string, mixed> $metadata Additional metadata
     * @return \Toporia\Framework\Audit\Contracts\AuditEntry|null
     */
    function audit_record(
        ...
        array $newValues = [],
        array $metadata = []
    ): ?\Toporia\Framework\Audit\Contracts\AuditEntry {
        return audit()->record($model, $event, $oldValues, $newValues, $metadata);
    }
}

if (!function_exists('without_auditing')) {
    /**
     * Execute callback without audit logging.
     *
     * @template T
     * @param callable(): T $callback
     * @return T
    ...
    {
        return audit()->withoutAuditing($callback);
    }
}
